...
### Диффузия:
### Частицы в левой половине сосуда (x < 250 пикселей) окрашиваются в красный, в правой половине сосуда ( x >= 250 пикселей) окрашиваются в синий. Физически это один и тот же газ (аргон), но благодаря разным цветам лучше получается отслеживать перемешивание.
### В окне графиков на панели «Динамика давления и диффузии» вместе с красной линией давления появляется зеленая кривая - коэффициент смешивания.
### Она показывает долю частиц, которые пересекли центральную границу и оказались в другой половине:
### -0 - газы полностью разделены (начальное состояние);
### - приблизительно 0.5 - полное равномерное перемешивание, диффузионное равновесие.
### Скорость роста этого коэффициента показывает влияние температуры на диффузию: чем выше температура, тем быстрее кривая достигает 0.5. Рекомендуемые параметры для наблюдения: 300-500 частиц, температура 500-800 K.

## Как пользоваться приложением
### после запуска приложения Вы можете выбрать количество частиц для моделирования и и температуру (от чег зависит начальнуая скорость частиц) в соответсвующих окошках для этого. Рекомундуемое количество частиц - 100-500, температура - 1-1000.
...
### -	Нажимая на кнопкy «Открыть окно графиков», пользователь будет открывать графики распределения скоростей и давления. График «Давления» показывает зависимость давления от времени. Оба графика обновляются в реальном времени, увидеть новых график пользователь может закрытием окна с графиком и повторным нажатием кнопки. 
...
# Авторы
## Чжан Епин (ИСУ: 505459) - диффузия, окна графиков
## Кавалерова Мария (ИСУ: 502764) - физические расчеты, структура функций
## Лотошвили Арина (ИСУ: 502380) - основной код, ООП, интерфейс
...
### -	Библиотеки Matplotlib, NumPy, Pygame, PyQT5
